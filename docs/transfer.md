...
- **เสร็จสิ้น (Done) tab** lists `complete` orders **and** short-ship waves where everything prepared so far has been received (`receive_caught_up`) — so receivers can print barcodes even when HQ still owes more qty on the same request. When HQ ships the remainder, the request returns to Active as `awaiting_receive`.
...
Sticker fields come from the receiving/printing site's ICMAS (`TRANSFER_SITE`). Layout: full-width Code 128 (10 mm tall), human-readable BCODE, then `{LOCATION1} | {HQ|SYP}` (print-site branch + ที่เก็บ). Also: ยี่ห้อ, หน่วย, บริษัท, รุ่น, เบอร์โรงงาน, รหัสสินค้า, ชื่อสินค้า, รหัสราคาทุน/ขาย, เบอร์แท้. ชื่อย่อ (ACODE) is not printed. Price letters: M0 P1 T2 N3 L4 B5 V6 S7 R8 C9 — `O` = cost, `X` = sell (270/420 → `OTSMXLTM`).
