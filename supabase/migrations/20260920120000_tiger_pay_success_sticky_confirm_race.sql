-- Prevent a late Tiger "fail" webhook (confirm race) from overwriting success
-- on payment_transaction. Companion attempts are already sticky; mirror that
-- for the transaction row so daily totals stay correct.
--
-- Also repair rows already stuck in fail after a confirm-mismatch race.
...
-- One-shot repair for confirm-race fails already stored.
update tiger_pay.payment_transaction
set
    status = 'success',
    remark = null,
    payload = case
        when payload ? 'payment' then
            jsonb_set(
                jsonb_set(payload, '{payment,status}', '"success"', true),
                '{payment,remark}',
                'null'::jsonb,
                true
            )
...
where lower(status) in ('fail', 'failed')
  and remark ilike '%does not match the expected value%';

update tiger_pay.payment_attempt
set error_message = null
where status = 'success'
  and error_message ilike '%does not match the expected value%';
